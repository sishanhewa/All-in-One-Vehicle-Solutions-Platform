...
const EditListing = () => {
  ...
  const loadListing = async () => {
    try {
      const API_URL = 'http://192.168.1.9:5000/api/marketplace';
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await updateListingAPI(listingId, formData);
  ...
  return (
    <ScrollView style={[styles.container, { paddingTop: insets.top }]} contentContainerStyle={styles.contentContainer}>
      ...
      <Text style={styles.subtitle}>Update your vehicle details</Text>
        ...
        <View style={styles.fieldGroup}>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false}>
            {existingImages.map((img, i) => (
              <Image key={i} source={{ uri: resolveImageUrl(img) }} style={styles.previewImage} />
            ))}
          </ScrollView>
        </View>
      )}
      ...
      {/* New Images */}
      <View style={styles.fieldGroup}>
        <Text style={styles.label}>Replace Photos</Text>
        <ScrollView horizontal showsHorizontalScrollIndicator={false}>
          {newImages.map((uri, i) => (
      ...
      <TouchableOpacity style={styles.submitBtn} activeOpacity={0.8} onPress={handleSubmit} disabled={submitting}>
        {submitting ? (
...
const styles = StyleSheet.create({
  ...
  previewImage: { width: 100, height: 100, borderRadius: 12, resizeMode: 'cover', marginRight: 8 },
  ...
  submitBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, backgroundColor: '#e67e22', paddingVertical: 16, borderRadius: 14, marginTop: 8, ...Platform.select({ ios: { shadowColor: '#e67e22', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.3, shadowRadius: 8 }, android: { elevation: 4 } }) },
